...
#### 17. Money 회고
`Money` 예제를 만든 과정과 그 결과를 다시 보자.<br>
1. 다음에 할일은 무엇인가?
2. 메타포 - 셀계 구조에 미치는 메타포의 엄청난 영향.
3. JUnit 사용도 - 언제 테스트를 실행했으며, JUnit을 어떻게 사용해 왔는가.
4. 코드 메트릭스 - 결과 코드의 수치화.
5. 프로세스 - 빨강/초록/리팩토링 순서에 대해 이야기 했는데, 각 단계에서 얼마 만큼 작업해야 하는가.
6. 테스트의 질 - 전통적인 테스트 메트릭스에 TDD 테스트가 어떻게 필적할 수 있는가.

<br>

* 최종 검토 <br>
1. 테스트를 확실히 돌아가게 만드는 세 가지 접근법 : 가짜로 구현하기, 삼각측량법, 명백하게 구현하기
2. 설계를 주도하기 위한 방법으로 테스트 코드와 실제 코드 사이의 중복을 제거하기.
3. 길이 미끄러우면 속도를 줄이고 상황이 좋으면 속도를 높이는 식으로 테스트 사이의 간격을 조절할 수 있는 능력.
